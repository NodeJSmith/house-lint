...
**Caching**: Per-directory compiled pattern tuples cached by directory path (`own_matcher_cache`). The fused stack-build-and-check function re-walks the ancestor chain per call, but each level's pattern tuple is a cache hit — no re-parsing or re-compilation. The combined-spec cache and spec-by-lines cache are deleted along with the flattening logic.
