...
Affected files:
- src/house_lint/cli.py:324-325
- src/house_lint/cli.py:348-350
...
(`src/house_lint/discovery.py:304-339`). `ScanResult.__post_init__`
(`src/house_lint/results.py:93-96`) only calls `.absolute()`, which does not collapse `..`
...
Add `.resolve()` alongside `.expanduser()` on both `src/house_lint/cli.py:324-325` and the
`except ConfigError` fallback derivation at `cli.py:348-350`, matching `resolve_project`'s
...
(`tests/integration/test_cli.py:266`) to assert exact equality (matches sibling tests) and adding a
