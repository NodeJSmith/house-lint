...
- File discovery does **not** shell out to git. It reads the root `.gitignore` plus every nested
  ...
  `Scenario` entry and needs no expected-value literal. Discovery uses a per-directory matcher
  stack (the walker supplies `is_dir` to each pattern) rather than delegating whole-path matching
  to `pathspec`'s aggregate spec, which previously caused divergences on directory-only negations.
  The parity and fuzz suites both show 0% divergence from `git check-ignore`. See
